...
int main()
{
		...
		function<void()> reset_ratelimit = [&]() {
			while (true) {
				...
				for (auto& find : members)
					if (find.second.ratelimit > 2) sleep_for(6s), find.second.ratelimit = 0, find.second.once_retelimit = false;
			}
		};
	...
	bot.on_message_create([](const dpp::message_create_t& event) {
		...
		if (commands::find_command_with_prefix(event.msg.content, event.msg.guild_id) not_eq -1) {
			ratelimit<const dpp::message_create_t&>(event);
			for (auto& find : members) if (find.second.ratelimit < 3) commands::commands_executed.emplace_back(commands::await_on_message_create, event);
		}
		});
	bot.on_slashcommand([](const dpp::slashcommand_t& event) {
		if (event.command.member.get_user()->is_bot() or event.command.member.get_user()->is_verified_bot()) return;
		if (slashcommand::find_command(event.command.get_command_name()) not_eq -1) {
			ratelimit<const dpp::slashcommand_t&>(event);
			for (auto& find : members) if (find.second.ratelimit < 3) slashcommand::slashcommands_executed.emplace_back(thread::thread(slashcommand::await_on_slashcommand, event));
		}
		});
