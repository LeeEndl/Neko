...
int main()
{
	...
	if (not ifstream("token").is_open())
		print<string>("Empty Token.", nullptr, state{ newline, color::red }),
		print<string>("token: ", [](string it) { bot.token = it; }, state{ Inline, color::white }), ofstream("token").write(bot.token.c_str(), streamsize(bot.token.size()));
