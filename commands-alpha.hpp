...
template<typename event_t> bool profile_t(event_t event)
{
	...
	string sort = "";
	for (auto& tool : data.tools) sort += (tool.name.find("B:") or tool.durability < 1 ? "Broken " + tool.name : "Durability: " + to_string(tool.durability) + " " + tool.name);
...
bool isCommand(const dpp::message_create_t& event) {
	GuildData g_data = GetGuildData(event.msg.guild_id);
	if (event.msg.content.find(g_data.prefix + "prefix ") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "daily") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "profile ") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "shop") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "buy ") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "sell ") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "fish") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "repair ") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "leaderboard") not_eq -1 or event.msg.content.find(g_data.prefix + "top") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "purge ") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "membercount") not_eq -1)  return true;
	else if (event.msg.content.find(g_data.prefix + "avatar ") not_eq -1 or event.msg.content.find(g_data.prefix + "avatar") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "invite") not_eq -1) return true;
	else if (event.msg.content.find(g_data.prefix + "hunt") not_eq -1) return true;
	else return false;
}
...
inline void await_on_slashcommand(const dpp::slashcommand_t& event) {
	...
	else if (event.command.get_command_name() == "avatar " or event.command.get_command_name() == "avatar") async(avatar_t<const dpp::slashcommand_t&>, event);
